...
_fhistory_select() {
	...
	# `next` keeps it against the list instead of pinned to the screen top.
	local pwin="next,1,wrap,noinfo,hidden"
	...
	local off="change-preview-window(next,1,wrap,noinfo,hidden)"
	...
	local fit="[ -f $qstate ] && { echo \"$off\"; exit; }; n=\$(zhis get -id $idf | awk -v w=\"\$FZF_COLUMNS\" '$wrapped') || { echo \"$off\"; exit; }; echo \"change-preview-window(next,\$n,wrap,noinfo)\""
	local id
